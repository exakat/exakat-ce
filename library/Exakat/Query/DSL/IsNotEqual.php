...
class IsNotEqual extends DSL {
    ...
    private function makeCommand(int|string|array $value): Command {
        // It is an integer
        if (is_int($value)) {
            return new Command('is(neq(' . (string) $value . '))');
        }

        // It is an array
        if (is_array($value)) {
            // @todo : also checks for the content of the array : type is int? string ? hash?
            return new Command('is(without(***))', array_values($value));
        }

        // It is a gremlin variable
        if ($this->isVariable($value)) {
            return new Command('is(neq(' . $value . '))');
        }

        // It is a label
        if ($this->isLabel($value)) {
            return new Command('where(neq("' . $value . '"))');
        }
