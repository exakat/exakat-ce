...
class Perrule extends Reports {
    ...
    public function _generate(array $analyzerList): string {
        ...
        foreach($analysisResults->toArray() as $row) {
            ...
            $title .= ' (https://exakat.readthedocs.io/en/latest/Reference/Rules.html#' . strtolower(strtr($row['analyzer'], '/', '-')) . ')';

            $perfile[$title][] = sprintf(' %-60s %-40s', $row['file'] . ':' . $row['line'], $row['fullcode']);
            $this->count();
        }
